...
#Import train data
with open('userdata_train_all_Nov.pkl', 'rb') as train:
    data_train = pickle.load(train)

#Import test data
with open('userdata_test_all_Nov.pkl', 'rb') as test:
    data_test = pickle.load(test)

model = GRU_model(vector_size=522,hidden_size=200,num_layers=2,output_size=43419).to(device)
model.load_state_dict(torch.load("best-4368-nov-125wei.mdl"))#Load the last optimal parameter result and use it directly
