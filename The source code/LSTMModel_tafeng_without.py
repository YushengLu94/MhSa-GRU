...
class GRU_model(nn.Module):
    def __init__(self, vector_size, hidden_size, num_layers, output_size):
        ...
        #embedding 操作
        self.embedding_work_product = nn.Embedding(23812, 500)
        self.embedding_work_category = nn.Embedding(2012, 400)
        #self_attention操作
        #self.attention_product = selfattention(500, 10, 0.1).to(device)
        #self.attention_category = selfattention(400, 5, 0.1).to(device)
        self.gru=nn.GRU(vector_size, hidden_size, num_layers, batch_first=True,dropout=0.2)
        self.fc1 = nn.Linear(hidden_size,8192)
        self.fc2 = nn.Linear(8192, output_size)
        self.relu = nn.ReLU()
